	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx 24rpx 80rpx","position":"relative","background":"#eefcff","height":"auto"}'>
		<form :style='{"padding":"24rpx 48rpx","boxShadow":"0 4rpx 12rpx #76c4d880","borderRadius":"60rpx","background":"#fff","display":"block","width":"100%","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#76c4d8","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}' class="title">食谱分类</view>
				<input :style='{"border":"0","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.shipufenlei" v-model="ruleForm.shipufenlei" placeholder="食谱分类"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shipufenlei`, this.ruleForm.id);
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.shipufenlei)){
					this.$utils.msg(`食谱分类不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`shipufenlei`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`shipufenlei`, this.ruleForm);
						}else{
							await this.$api.add(`shipufenlei`, this.ruleForm);
						}
