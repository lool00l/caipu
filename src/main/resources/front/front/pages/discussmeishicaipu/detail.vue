...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussmeishicaipuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussmeishicaipuCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussmeishicaipu', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussmeishicaipu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussmeishicaipu');
			...
			async init(){
				...
				let res = await this.$api.info('discussmeishicaipu', this.id);
				this.detail = res.data;
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussmeishicaipu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussmeishicaipu/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussmeishicaipu',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussmeishicaipu',that.detail)
							setTimeout(()=>{
